...
class ElmNode(template.Node):
    ...
    def compile(self, source):
        process = Popen(ELM_COMPILER_EXECUTABLE, shell=True, stdin=PIPE,
                        stdout=PIPE)
        (javascript, err) = process.communicate(
            input=self.nodelist.render(source))
        if err:
    ...
    def render(self, context):
        return '<script type="text/javascript">' + self.compile(context) + '\
                </script>'
